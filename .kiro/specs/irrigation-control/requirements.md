...
6. IF the status request fails, THEN THE IrrigationPage SHALL display an error message with a retry button that re-fetches the status.
...
8. IF no irrigation device exists in the system (irrigationDeviceProvider is null), THEN THE IrrigationPage SHALL display a message indicating that no irrigation device is registered.
...
3. THE irrigationDeviceProvider SHALL be a Provider that watches the devicesControllerProvider and exposes the first device whose type is DeviceType.irrigation, or null if no irrigation device exists.
...
6. IF irrigationDeviceProvider is null when the irrigationControllerProvider builds, THEN THE irrigationControllerProvider SHALL emit an error state indicating no irrigation device is available.
